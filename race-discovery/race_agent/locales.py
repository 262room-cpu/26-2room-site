...
_months(5, "мая", "май", "may", "mai", "мамыр", "мамырдың", "мая", "մայիս", "მაისი")
...
CURRENCY_PATTERN = "|".join([
    ...
    r"AMD", r"֏", r"dram", r"drams", r"драм(?:ов|а)?",
    r"BYN", r"бел\.??\s*руб\.??",
    ...
    r"MNT", r"₮", r"tugrik", r"tugriks", r"төгрөг",
])
...
def parse_dates(text: str, year_min: int = 2026, year_max: int = 2028) -> list[str]:
    ...
    Day-first is the default for ambiguous numeric dates because all current target markets use
    that convention on race pages. ISO year-first is handled separately.
    Date ranges add the first day before the last day so an event edition gets its opening date.
    """
    out: list[str] = []
    source = text or ""
    ...
    # 26 September - 27 September 2026
    ...
    # 27 September 2026 / 27 / September / 2026
    ...
    # Mongolian: 2026 оны 9-р сарын 27
...
def parse_prices(text: str) -> list[str]:
    ...
    for amount, currency in re.findall(rf"(?<!\d)({number})\s*({CURRENCY_PATTERN})\b", text or "", flags=re.I):
...
def focused_event_text(text: str, event_name: str, year_min: int = 2026, year_max: int = 2028) -> str:
    """Choose the occurrence of an event title that has a real dated event block nearby.

    Organizer sites often repeat a full race calendar in navigation before the current event body.
    A small date-bearing window around the title is a strong deterministic signal that we reached
    the current event card/body. If no such occurrence exists, return the original text unchanged.
    """
    ...
    for pos in positions:
        ...
        if re.search(r"\b(?:start|старт|registration|регистрац|distance|дистанц|route|маршрут)\b", local, flags=re.I):
            score += 2
        # In a tie prefer the later occurrence: navigation usually comes before the page body.
