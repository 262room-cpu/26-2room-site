...
def _has_primary_candidate(profile: dict, kind: str, value: str) -> bool:
    ...
    for row in profile.get("evidence", []):
        if row.get("source_type") not in {"official_event", "official_organizer", "federation", "government"}:
            continue
...
def apply_social_hit(profile: dict, event: dict, hit: SearchHit, observed_at: str) -> tuple[bool, bool, float]:
    """Corroborate a social route without using a social display name as organizer identity."""
...
def _hint_domain(hint: str) -> str:
    value = str(hint or "").strip().lower().removeprefix("https://").removeprefix("http://").removeprefix("www.")
...
def _official_domains(event: dict, profile: dict, cfg: dict) -> set[str]:
    ...
    for hint, source_type in cfg.get("source_hints", {}).items():
        if source_type in {"official_event", "official_organizer", "federation", "government"}:
            out.add(_hint_domain(hint))
    return {x for x in out if x}


def _copy_named_identity(profile: dict, resolved: dict, event: dict, source_url: str, observed_at: str) -> bool:
    name = str(resolved.get("name") or "").strip()
    if not name or resolved.get("identity_status") != "NAMED":
        return False
    if profile.get("identity_status") == "NAMED" and normalize(profile.get("name")) == normalize(name):
        return False

    profile["name"] = name
    profile["display_name"] = name
    profile["identity_status"] = "NAMED"
    profile["identity_source"] = "PRIMARY_STRUCTURED_ORGANIZER"
    profile["confidence"] = max(float(profile.get("confidence", 0) or 0), float(resolved.get("confidence", 0) or 0), 0.85)
    profile.setdefault("evidence", []).append({
        "field": "name",
        "value": name,
        "url": source_url,
        ...
        "weight": 1.0,
        "association": "STRUCTURED_ORGANIZER_IDENTITY",
    })
    event["organizer"] = name

    # Contacts explicitly tied to the structured organizer are safe to carry over.
    for kind, values in (resolved.get("contacts") or {}).items():
        profile.setdefault("contacts", {}).setdefault(kind, [])
        profile["contacts"][kind] = _unique(profile["contacts"][kind] + list(values or []))
    return True


def apply_official_web_hit(profile: dict, event: dict, hit: SearchHit, cfg: dict, observed_at: str) -> tuple[bool, int, bool]:
    """Verify contacts and, when primary structured evidence exists, resolve organizer identity."""
    score = relation_score(hit, event, profile)
    hit_domain = domain(hit.url)
    ...
    if source_type in {"official_event", "official_organizer", "federation", "government"}:
        resolved = organizer_lead_from_document(event, raw, hit.url, source_type, observed_at)
        if resolved and resolved.get("identity_status") == "NAMED":
            identity_resolved = _copy_named_identity(profile, resolved, event, hit.url, observed_at)
            org_name = normalize(str(profile.get("name") or ""))
    ...
    for kind in ("email", "phone", "whatsapp"):
        for value in candidates.get(kind, []):
            profile.setdefault("contacts", {}).setdefault(kind, [])
            before = set(profile["contacts"][kind])
            profile["contacts"][kind] = _unique(profile["contacts"][kind] + [value])
            ...
            profile.setdefault("evidence", []).append({
                ...
                "observed_at": observed_at,
                "weight": 0.9 if source_type in {"official_event", "official_organizer"} else 0.75,
                "public_contact": True, "association": "PRIMARY_SOURCE_OUTREACH_CONTACT",
            })

    # Social links are promoted only on a named organizer-owned page. On event pages they remain
    # candidates because sponsors and partners commonly appear in the same footer/body.
    ...
    return True, promoted, identity_resolved
...
def research_market(code: str, cfg: dict, max_tasks: int = 8, queries_per_task: int = 3, results_per_query: int = 6) -> dict:
    ...
    if not queue:
        return {"market_code": code, "tasks_processed": 0, "search_hits": 0, "candidates_found": 0, "contacts_promoted": 0, "identities_resolved": 0}
    ...
    evidence_rows = []
    tasks_processed = hits_seen = found = promoted = identities_resolved = 0
    web_fetches = 0
    ...
    for task in queue:
        ...
        for query in task.get("search_queries", [])[:queries_per_task]:
            ...
            for hit in hits:
                ...
                if score < 0.58:
                    continue
                record = {
                    "observed_at": observed, "market_code": code, "task_id": task.get("task_id"),
                    "candidate_id": event.get("candidate_id"), "organizer_id": task.get("organizer_id"),
                    "query": query, "url": hit.url, "title": hit.title, "snippet": hit.snippet[:800],
                ...
                if profile:
                    ...
                    if not _social_contact(hit.url)[0] and web_fetches < max_web_fetches:
                        used, count, identity_resolved = apply_official_web_hit(profile, event, hit, cfg, observed)
                        if used:
                            web_fetches += 1
                            promoted += count
                            task_promoted += count
        ...
        if profile:
            ...
            elif task_identity_resolved:
                task["status"] = "IDENTITY_RESOLVED_CONTACTS_MISSING"
        ...
        task["identity_resolved"] = bool(task.get("identity_resolved") or task_identity_resolved)

    # A resolved identity can change organizer/contact fields on the event immediately; the next
    # main discovery pass will canonicalize/deduplicate the organizer ID across multiple events.
    by_org = {o.get("organizer_id"): o for o in organizers if o.get("organizer_id")}
    for event in events:
        ...
        if profile:
            ...
            if profile.get("identity_status") == "NAMED" and profile.get("name"):
...
def main() -> int:
    ...
    if args.market:
        selected = args.market
    elif args.all:
        selected = [code for code, _ in enabled_markets(CONFIG)]
        if args.max_markets > 0:
            selected = selected[:args.max_markets]
    else:
        selected = choose_research_markets(args.max_markets)
