...
class OrganizerQueueTests(unittest.TestCase):
    ...
    def test_previous_research_history_survives_queue_rebuild(self):
        ...
        # Legacy researched queues migrate to the second batch instead of repeating batch one.
        self.assertEqual(row["query_offset"], 2)

    def test_successive_runs_rotate_first_query(self):
    ...
    def test_deep_research_eventually_checkpoints_for_review(self):
        ...
        for i in range(12):
            row = build_progressive_organizer_research_queue(
                [self.event()], [self.unresolved()], previous, f"run-{i}", f"2026-09-{10+i:02d}T00:00:00Z"
            )[0]
        ...
        # A rebuilt exhausted task carries a machine-readable review checkpoint.
