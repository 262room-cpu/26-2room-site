...
begin
  ...
  v_participant_last_name :=
    pg_catalog.btrim(p_participant_last_name);

  v_participant_first_name :=
    pg_catalog.btrim(p_participant_first_name);

  v_participant_middle_name :=
    nullif(pg_catalog.btrim(p_participant_middle_name), '');

  v_participant_gender :=
    pg_catalog.lower(pg_catalog.btrim(p_participant_gender));

  v_participant_phone_display :=
    pg_catalog.btrim(p_participant_phone);
  ...
  v_participant_email_display :=
    pg_catalog.btrim(p_participant_email);

  v_participant_email_normalized :=
    pg_catalog.lower(v_participant_email_display);

  v_event_rules_consent_version :=
    pg_catalog.btrim(p_event_rules_consent_version);

  v_personal_data_consent_version :=
    pg_catalog.btrim(p_personal_data_consent_version);

  v_health_responsibility_consent_version :=
    pg_catalog.btrim(p_health_responsibility_consent_version);
  ...
  v_event_local_date :=
    (v_event_starts_at at time zone v_event_timezone)::date;
  ...
  v_reservation_expires_at :=
    v_reserved_at + interval '15 minutes';
  ...
  if
    v_event_capacity is not null
    and v_event_occupied >= v_event_capacity
  then
    raise exception using
      errcode = 'P0001',
      message = 'sold_out';
...
comment on function public.room262_create_adult_registration_reservation(
  ...
  'Server-only atomic reservation for self-registering adult participants. Requires age 18 or older on the event date.';
